...
from features import LEAK, TREND, case_features, case_labels
...
def train(meta: pd.DataFrame, seed: int = 0, max_train_rows: int = 400_000):
    ...
    naive_te = np.clip((MAP_THR + 5 - Xte["map_now"]) / 20.0, 0, 1)

    aucs = {
        "trend": roc_auc_score(yte, m_trend.predict_proba(Xte[TREND])[:, 1]),
        "leak": roc_auc_score(yte, m_leak.predict_proba(Xte[LEAK])[:, 1]),
        "naive_currentMAP": roc_auc_score(yte, naive_te),
        "event_rate": float(yte.mean()),
    }
    return {"trend": m_trend, "leak": m_leak}, aucs
